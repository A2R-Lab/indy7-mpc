...
def figure_8(x_amplitude=0.4, z_amplitude=0.5, pos_offset=[0.0, 0.6, 0.5], timestep=0.01, period=10, num_periods=10, theta=0.0, axis='z'):
    x = lambda t: pos_offset[0] + x_amplitude * np.sin(t)  # [-x_amplitude, x_amplitude]
    y = lambda t: pos_offset[1]
    z = lambda t: pos_offset[2] + z_amplitude * np.sin(2*t)/2 + z_amplitude/2  # [-z_amplitude/2, z_amplitude/2]
